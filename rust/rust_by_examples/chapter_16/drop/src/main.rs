// Drop is another trait, which again implements drop.
// We could let rust drop for us, but sometimes we want to drop ahead of schedule
// and so we can implement Drop ourselves and drop ahead of schedule
use std::fs::File;
